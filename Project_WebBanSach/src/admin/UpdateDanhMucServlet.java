...
/**
 * Servlet implementation class UpdateDanhMucServlet
 */
public class UpdateDanhMucServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public UpdateDanhMucServlet() {
		super();
	...
	private void toDo(HttpServletRequest request, HttpServletResponse response)
		...
		if (tendm == "") {
			...
			try {
				...
				if (count == 0) {
					DanhMucDao.updateDanhMuc(Integer.parseInt(madm), tendm);
				} else {
		...
		if (baoloi = false) {
			response.sendRedirect(tenproject + "/admin/UpdateDanhMuc.jsp");
		} else {
			request.setAttribute("tendm_err", tendm_err);
			RequestDispatcher rd = getServletContext().getRequestDispatcher(
					"/admin/UpdateDanhMuc.jsp");
